...
### 5. Ordering — DONE (0.2.0)

Shipped as a rank per compiled rule: `compileOne` returns `{ css, rank }` and the DOM engine
keeps its rules array sorted (binary-search insert, one textContent rewrite per add() that
inserted something — same O(total) as appending was). The rank packs two components:

- **Variant key** (dominant): v4's variant order from `getVariants()`, compound families ranked
  by base name, named values before arbitrary ones within a family (`data-active` <
  `data-[state=open]`), stacks compared by their top three ranks descending.
- **Utility key**: v4 sorts utilities by the CSS properties they emit, so a utility's property
  signature (first two declared properties, `~`-prefixed for child-selector utilities like
  `divide-*`) names its slot. `scripts/gen-ranks.mjs` walks the oracle's full class list in
  `getClassOrder` order, records each signature's position interval, and merges overlapping
  intervals — families v4 interleaves (text-shadow sizes/colors) become shared ranks, i.e.
  ties, instead of coin flips. Arbitrary properties (`[color:red]`) get exact placement for
  free: their property IS the key.

Verified by `conformance/ordering.test.mjs` against `getClassOrder` bigint keys: 131,018 /
131,018 ordered pairs agree on the FULL sweep (ties carry no claim and are reported
separately), colight corpus exact, ratchet at 1.0. Cost: engine 51.0→59.3 KB min,
16.0→18.3 KB gz — the canonical-order tables (`src/core/ranks.ts`, generated) plus insertion
logic. A substring dictionary was measured and rejected: −1.2 KB min but only −60 B gz.
